...
local function highlight(parent: Instance, color: Color3, text: string, stroke: Color3?, twisted: boolean?)
	...
	local function addBillboard(p: Instance, offset: Vector3?)
		local BillboardGui: BillboardGui =	createInstance("BillboardGui", p, { Adornee = p, AlwaysOnTop = true, Name = "I.C.H.O.R. | BillboardGui", Size = UDim2.fromOffset(200, 50), StudsOffsetWorldSpace = offset or Vector3.zero, ZIndexBehavior = Enum.ZIndexBehavior.Sibling, ClipsDescendants = true })
...
local function PlayerHighlight(parent: Instance, name: string, heart: number, slot: StringValue?)
	...
	if not billboardGui then
		local billboardGui: BillboardGui =	createInstance("BillboardGui", parent, { Adornee = parent, AlwaysOnTop = true, Name = "I.C.H.O.R. | BillboardGui", Size = UDim2.fromOffset(150, 100), StudsOffsetWorldSpace = Vector3.new(0, 4, 0), ZIndexBehavior = Enum.ZIndexBehavior.Sibling, ClipsDescendants = true })
		billboardGui:AddTag(cache.TAG)
